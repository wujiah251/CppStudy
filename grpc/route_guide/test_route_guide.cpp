...
#include "../protos/route_guide/route_guide.grpc.pb.h"
...
void RouteGuideClient::RecordRoute() {
    ...
    if (status.ok()) {
        std::cout << "Finish trip with " << stats.point_count() << " points\n"
            ...
            << "Ravelled " << stats.distance() << " meters\n"
...
static void test_get_feature(benchmark::State& state)
{
    auto chan = grpc::CreateChannel("localhost:50051",grpc::InsecureChannelCredentials());
    RouteGuideClient client(chan,"");
    for(auto _:state)
    {
        client.GetFeature();
    }
